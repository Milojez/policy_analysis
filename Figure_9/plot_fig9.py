#This script generates figure 9 from ground-truth fixations (all videos).
import os
import sys
import numpy as np
...
TARGET_VIDEO = 7
VIDEO_IDX    = TARGET_VIDEO - 1   # 0-based

# ------------------ USER SETTINGS ------------------

CATS_CSV   = os.path.join(config._ROOT, "data", "CatS_from_fixations_noDial7.csv")
SAVE_FIG   = os.path.join(_HERE, f"fig9_GT_video{TARGET_VIDEO}.png")
PLOT_TITLE = f"Figure 9 (Ground truth, video {TARGET_VIDEO}): AOI vs Angle, Velocity, and Time to Crossing"

CATS_ARE_BW    = False     # True if cat already = bandwidth rank (1..6)
USE_MATLAB_STD = True      # True to match MATLAB std() behavior
MIN_SAMPLES    = 50        # min frames per bin (lower than 250 — single video)

# ---------------------------------------------------

# 1) load CatS and convert to bandwidth rank
CatS  = load_cats_csv(CATS_CSV)
CatSb = CatS if CATS_ARE_BW else dialpos_to_bw_rank(CatS)

# 2) CatHb for video 7 only: (4500, 6)
...
sig_v7 = signal_for_dial(90.0, rg=TARGET_VIDEO, use_matlab_std=USE_MATLAB_STD)  # (4500, 6)

# 4) Compute Figure 9 curves (single-video)
fps   = 50
temp2 = sig_v7
temp3 = CatHb_v7
temp4 = np.vstack([np.full((1, 6), np.nan), np.diff(temp2, axis=0) * fps])
temp5 = temp2 / (-temp4)
